...
namespace {
cbf_arbitration::CbfParam makeDefaultParam() {
  cbf_arbitration::CbfParam param;
...
int main() {
  ...
  const cbf_arbitration::CbfParam param = makeDefaultParam();

  // TC1: 远处障碍物
  {
    totalCount += 1U;
    cbf_arbitration::EgoState ego;
    ego.velocity = 5.0;
    ego.aOriginal = 1.0;
    ego.deltaFOriginal = 0.0;
    cbf_arbitration::ObstacleState obs[1];
    obs[0].dxEgo = 80.0;
    obs[0].dyEgo = 0.0;
    obs[0].vRxEgo = 0.0;
    obs[0].vRyEgo = 0.0;
    const cbf_arbitration::CbfOutput out = cbf_arbitration::cbfControlRevision(ego, obs, 1U, param);
  ...
  // TC3: 无障碍物
  {
    ...
    const cbf_arbitration::CbfOutput out = cbf_arbitration::cbfControlRevision(ego, nullptr, 0U, param);
