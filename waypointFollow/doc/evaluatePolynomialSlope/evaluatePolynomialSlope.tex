...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small evaluatePolynomialSlope~---~函数设计文档}
...
\hline
\textbf{函数英文名} & evaluatePolynomialSlope &
\textbf{函数中文名} & 多项式斜率求值 \\
\hline
...
\texttt{evaluatePolynomialSlope} 计算三次多项式在给定点 $x$ 处的一阶导数（斜率）：
\[
  \frac{dy}{dx} = c_1 + 2c_2 x + 3c_3 x^2
\]
采用 Horner 法优化计算效率。
...
对三次多项式 $y = c_0 + c_1 x + c_2 x^2 + c_3 x^3$ 求一阶导数：
\begin{equation}
  \frac{dy}{dx} = c_1 + 2c_2 x + 3c_3 x^2
\end{equation}

这是一个二次多项式。采用 Horner 法改写为嵌套形式：
\begin{equation}
  \frac{dy}{dx} = c_1 + x \cdot (2c_2 + 3c_3 \cdot x)
\end{equation}

\textbf{计算步骤}：
\begin{enumerate}
  \item $t_1 = 2 \cdot c_2 + 3 \cdot c_3 \cdot x$
  \item $\text{slope} = c_1 + x \cdot t_1$
\end{enumerate}

Horner 形式仅需 3 次乘法和 2 次加法（直接展开需 4 次乘法和 2 次加法）。

\textbf{注意}：$c_0$ 为常数项，求导后消失，不参与斜率计算。
...
\node (calc) [process, below=of input] {slope = c[1] + x*(2*c[2] + 3*c[3]*x)};
\node (output) [process, below=of calc] {返回 slope};
...
\hline
\endhead
函数英文名 & \texttt{evaluatePolynomialSlope} \\
\hline
函数中文名 & 多项式斜率求值 \\
\hline
函数类型 & 元件 (Element) \\
...
版本 & 2.0 \\
\hline
源文件 & \texttt{src/cpp/evaluatePolynomialSlope/evaluatePolynomialSlope.cpp} \\
\hline
头文件 & \texttt{include/cpp/evaluatePolynomialSlope/evaluatePolynomialSlope.hpp} \\
\hline
...
无下级函数调用。\texttt{evaluatePolynomialSlope} 为叶节点元件，仅包含算术运算。
...
N-4 &
线性多项式 &
$\mathbf{c}{=}[5,3,0,0]$, $x{=}100$ &
$s = 3.0$ &
$c_2{=}c_3{=}0$ 时斜率为常数 $c_1$ \\
\hline
N-5 &
小数值 $x$ &
$\mathbf{c}{=}[0,0,1,0]$, $x{=}0.5$ &
$s = 1.0$ &
$2 \cdot 1 \cdot 0.5 = 1.0$ \\
\hline
\end{longtable}
}
...
A-4 &
$c_1$ 为 NaN &
$\mathbf{c}{=}[0,\text{NaN},3,4]$, $x{=}2$ &
$s = \text{NaN}$ &
NaN 系数传播 \\
\hline
A-5 &
$c_2$ 为 $\infty$ &
$\mathbf{c}{=}[0,0,+\infty,0]$, $x{=}1$ &
$s = +\infty$ &
$2 \cdot \infty \cdot 1 = +\infty$ \\
\hline
A-6 &
$\infty - \infty$ 抵消 &
$\mathbf{c}{=}[0,+\infty,0,-\infty]$, $x{=}1$ &
$s = \text{NaN}$ &
$+\infty + (-\infty) = $ NaN \\
\hline
A-7 &
...
A-8 &
溢出场景 &
$\mathbf{c}{=}[0,0,0,1]$, $x{=}10^{154}$ &
$s = +\infty$（溢出） &
$3 \cdot 10^{308} > $ DBL\_MAX \\
\hline
A-9 &
全零系数 &
